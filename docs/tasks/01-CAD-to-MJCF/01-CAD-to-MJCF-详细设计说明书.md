...
```
assets/mjcf/
└── electronbot.xml     ← 唯一输出: inline mesh 版，CAD 真实外形
...
|------|---------------------|-----------------|
| 输出文件名 | `electronbot_mesh.xml` | `electronbot.xml` |
...
该脚本按舵机规格重新写入 `electronbot.xml` 的 actuator 段：
...
```bash
python -c "import mujoco; m=mujoco.MjModel.from_xml_path('assets/mjcf/electronbot.xml'); print(m.njnt,'joints,',m.nu,'actuators')"
```
...
- `electronbot.xml` 自带光照和相机，可直接在 viewer 中观察
...
| 文件 | 描述 | 生成方式 |
|------|------|----------|
| `assets/mjcf/electronbot.xml` | inline mesh 版 MJCF（~1.7MB） | `scripts/generate_inline_mesh.py` |
