...
>
> **前置依赖**：Phase 6 完成（传感器+观测空间）
>
> **真机部署**：训练完成的策略通过 [Sim2Real 真机部署](../09-Sim2Real/09-Sim2Real-详细设计说明书.md) 部署到 ESP32-S3 真机。了解真机硬件能力参见 [硬件规格](../../概要设计/ElectronBot_SIM-概要设计文档.md#8-真机对接)。
>
> **输出**：`src/electronbot_ai/`——训练脚本、数据集收集工具、策略评估脚本
>
> **文档版本**: v1.3  
> **最后更新**: 2026-07-08  
> **变更类型**: VLA LLM 改用本地 ollama + qwen2.5:7b，新增安装脚本

---

## 整体架构中的位置

Phase 7（AI 训练管线）是 ElectronBot-SIM **9 Phase 全链路** 中的**智能决策层**，在仿真中训练模仿学习（IL）、强化学习（RL）、视觉语言动作（VLA）三条 AI 管线。

- **上游依赖**：Phase 6（Sensors）——观测空间、传感器数据是策略训练的基础输入
- **下游支撑**：Phase 8（Benchmark）——训练好的策略提交给评估系统测试；Phase 9（Sim2Real）——策略通过 MCP 接口直接部署到真机
- **核心价值**：这是项目的 AI 核心——三条路径覆盖从"模仿人类操作"到"自主探索学习"到"语言指令驱动"的完整 AI 能力谱系
...
graph LR
    ...
    style P7 fill:#a5d6a7,stroke:#2e7d32,stroke-width:4px,color:#1b5e20
...
```mermaid
graph TD
    A["📋 定义 7 个训练任务<br/>EB-Reach/Push/PickPlace<br/>Stack/Follow/Gesture/VoiceCmd"] --> B["📦 实现任务基类<br/>BaseTask<br/>reset/reward/is_success"]
    B --> C["📥 路径1: 模仿学习 IL<br/>键盘收集示范 → BC/ACT 训练"]
    B --> D["🏋️ 路径2: 强化学习 RL<br/>PPO + 域随机化<br/>64 环境并行"]
    B --> E["🧠 路径3: VLA 大模型<br/>ollama + qwen2.5:7b<br/>语音→动作序列"]
    C --> F["📊 策略评估<br/>成功率/完成时间<br/>Sim2Real Gap"]
    D --> F
    E --> F
    F --> G["🔄 真机验证<br/>部署评估"]
    G -->|"效果差"| C
    G -->|"通过"| H["✅ 训练完成<br/>checkpoint 输出<br/>Phase 8 Benchmark 待评估"]
...
> **LLM 后端**: 默认使用 **本地 ollama + qwen2.5:7b**，延迟 50-200ms。
> 无需 GPU 亦可运行（CPU 推理 ~5-10 tok/s），详见 [环境安装](#511-vla-llm-环境准备)。
...
class TextVLAPlanner:
    ...
    使用本地 ollama + qwen2.5:7b 推理，真机可用，延迟 50-200ms。
    qwen2.5:7b Q4_K_M 约 4.5GB 显存，RTX 2060 12GB 可流畅运行。
    """
    
    # ── 推理参数（可环境变量覆盖） ──
    DEFAULT_MODEL = "qwen2.5:7b"
    DEFAULT_TEMPERATURE = 0.1   # 低温度保证动作输出稳定
    DEFAULT_NUM_PREDICT = 512   # 最多输出 512 token（足够一个动作序列）
    ...
    def __init__(
        ...
        # 允许自定义 ollama 地址（默认 localhost:11434）
        if ollama_host:
        ...
        logger.info(f"TextVLAPlanner 初始化: model={self.model}, temp={self.temperature}")
    ...
    def _build_prompt(self, instruction: str) -> str:
        return f"""你是一个控制桌面机器人 ElectronBot 的动作规划器。

## 可用工具（真机可用）
- self.electron.hand_action:
  action(1=举手,2=放手,3=挥手,4=拍打), hand(1=左手,2=右手,3=双手),
  steps(1-100,越大越细腻), speed(500-2000), amount(10-90)
- self.electron.body_turn:
  direction(1=左转,2=右转,3=回中), steps, speed, angle(0-90°)
- self.electron.head_move:
  action(1=抬头,2=低头,3=点头,4=回中,5=连续点头), steps, speed, angle(1-15°)
- self.electron.stop: 紧急停止

## 输出规则
- 同时使用双手时 hand=3
- 打招呼/问好用 hand_action action=3 hand=3
- 告别/再见用 hand_action action=3 hand=3 steps=3
- 左转/右转用 body_turn
- 点头/摇头用 head_move
- 输出严格 JSON 数组，不要任何解释性文字
- 不要使用 servo_move 或 servo_sequences

## Few-shot 示例
用户: "你好" → {{
  "actions": [{{"tool": "self.electron.hand_action", "args": {{"action": 3, "hand": 3, "steps": 2, "speed": 600}}}}]
}}
用户: "转过来看我" → {{
  "actions": [{{"tool": "self.electron.body_turn", "args": {{"direction": 2, "angle": 45, "speed": 600}}}}]
}}
用户: "点点头" → {{
  "actions": [{{"tool": "self.electron.head_move", "args": {{"action": 3, "steps": 2, "speed": 400, "angle": 5}}}}]
}}

现在请根据用户指令「{instruction}」，输出动作序列 JSON：
{{"actions": [...]}}"""
    ...
    def _parse_response(self, response: str) -> dict:
        ...
        if not json_match:
            logger.warning(f"LLM 输出未找到 JSON: {response[:100]}")
            return {"actions": []}
    ...
    def execute(self, instruction: str):
        """端到端：语音指令 → 生成动作 → 执行（本地 ollama，延迟 50-200ms）"""
...
```python
# 仅用于仿真研究，真机不可用
# 视觉模型推荐 llava:13b 或 bakllava（需额外 ~8GB 显存）
...
class VisionVLAPlanner:
    ...
    推荐模型: llava:13b（约 8GB 显存）或 minicpm-v（约 6GB 显存）
    """
    ...
    def plan(self, instruction: str, camera_image: np.ndarray) -> dict:
        ...
        resp = ollama.chat(
            model=self.model,
            messages=[{"role": "user", "content": prompt, "images": [camera_image]}],
            options={"temperature": 0.1}
        )
        return self._parse_response(resp["message"]["content"])
...
```bash
# 一键安装 ollama + qwen2.5:7b
bash scripts/setup-vla-llm.sh
...
```
□ 指令: "举起右手" → LLM输出: {"actions":[{"tool":"self.electron.hand_action",...}]}
    执行后右手举起 ✓
□ 指令: "挥手打招呼" → LLM输出 hand_action action=3
    执行后机器人挥手 ✓
□ 指令: "转过来看我" → LLM输出 body_turn 序列
    执行后腰部旋转 ✓
□ 指令: "点点头" → LLM输出 head_move action=3
    执行后头部点动 ✓
□ 指令: "再见" → LLM输出挥手动作 (action=3, steps=3)
    执行后机器人挥手告别 ✓
```
...
```
□ ollama 服务运行正常（curl localhost:11434/api/tags）
□ 单次 plan() 调用延迟 < 200ms（GPU）或 < 2000ms（CPU）
□ JSON 解析成功率 > 95%（50 条不同指令测试）
□ 温度 0.1 下输出稳定（同一指令 5 次输出一致）
```
...
| 文件 | 描述 |
|------|------|
| `scripts/setup-vla-llm.sh` | VLA LLM 环境一键安装脚本（ollama + qwen2.5:7b） |
...
| `src/electronbot_ai/vla/llm_planner.py` | VLA 规划器（基于 ollama） |
| `src/electronbot_ai/tasks/` | 7 个标准任务定义 |
| `demos/` | 示范数据存储目录 |
...
class TextVLAPlanner:
    def __init__(
        ...
        temperature: float = 0.1,          # 推理温度（低温=更稳定）
        ollama_host: str = None,           # ollama 地址，默认 localhost:11434
    ): ...
    def plan(self, instruction: str) -> dict:
...
- **LLM 后端**：本地 ollama，推荐 `llava:13b` 或 `minicpm-v`
...
#### 11.1.6 VLA LLM 本地推理要求

| 资源 | 最低要求 | 推荐配置 | 说明 |
|------|---------|---------|------|
| ollama | latest | latest | 一键安装: `bash scripts/setup-vla-llm.sh` |
| GPU（可选） | GTX 1060 6GB | RTX 2060 12GB | GPU 推理延迟 50-200ms，CPU 推理 1-5s |
| 内存（纯 CPU） | 16GB | 32GB | qwen2.5:7b Q4_K_M CPU 推理约需 8GB |
| 显存（GPU） | 6GB | 8GB+ | qwen2.5:7b Q4_K_M 约 4.5GB 显存 |
| 磁盘 | 10GB | 20GB | 模型文件约 4.5GB |
| VLA 模型 | `qwen2.5:7b` | `qwen2.5:14b` | 7B 足够 VLA 任务，14B 仅边际提升 |
| 视觉模型（可选） | `llava:7b` | `llava:13b` | 仅视觉 VLA（仿真专属）需要 |
...
| `ELECTRONBOT_LLM_MODEL` | VLA 使用的 LLM 模型（ollama 名） | `qwen2.5:7b` | `qwen2.5:14b` |
| `ELECTRONBOT_OLLAMA_HOST` | ollama 服务地址 | `http://localhost:11434` | `http://192.168.1.100:11434` |
| `ELECTRONBOT_OLLAMA_MODEL` | 安装脚本默认模型 | `qwen2.5:7b` | 通过 `OLLAMA_MODEL` 环境变量覆盖 |
| `ELECTRONBOT_VLA_TEMPERATURE` | VLA 推理温度 | `0.1` | `0.3`（提高多样性） |
...
| v1.3 | 2026-07-08 | VLA LLM 改用本地 ollama + qwen2.5:7b；新增 `scripts/setup-vla-llm.sh` 一键安装脚本；TextVLAPlanner 实现改为 ollama.chat()；新增 few-shot prompt 提升稳定性；新增 VLA LLM 推理硬件要求表 | 架构组 |
