...
> 版本：v3.0
> 日期：2026-07-16（基于实际代码核对更新，反映v3.0自动化零件提取和外部STL引用方案）
...
│  Layer 1: 模型描述层                              ◄── 建模基础  │
│  ┌──────────────────────────────────────────────────────────┐ │
│  │  MJCF 运动学模型 (外部 STL 引用, 347 零件自动化提取)      │ │
│  │  ├── 7 body 运动学链 (base_link → body → head/arm)        │ │
│  │  ├── 6 铰链关节 (hinge, 含限位 range)                      │ │
│  │  ├── 6 位置执行器 (kp=5, kv=2, 抑制振荡)                   │ │
│  │  ├── 347 个独立零件 STL (cascadio + trimesh 自动提取)      │ │
│  │  ├── 坐标本地化 (世界坐标→父 body 相对坐标)                │ │
...
│   ├── cad/                    #   原始 CAD 模型 (ElectronBot.step, 30.5MB)
│   ├── meshes/                 #   STL 网格
│   │   └── step_parts/         #     347 个独立零件 STL (自动提取, gitignore)
│   └── mjcf/                   #   MuJoCo 模型文件
│       └── electronbot_step_meters.xml  # v3.0 主模型 (~103KB, 外部 STL 引用)
│
├── src/
...
│
├── scripts/                    # 工具脚本
│   ├── step_to_mjcf.py           v3.0 核心: STEP → MJCF 自动化转换
│   │                             (cascadio + trimesh, 347 零件提取 + 坐标本地化)
...
| | | PyBullet | 物理精度较低 |
| CAD→MJCF | **v3.0 自动化提取** (cascadio + trimesh) | FreeCAD 手动导出 | v2.0 手动导出 24 零件效率低，v3.0 自动提取 347 零件 |
...
| **MJCF 方案** | **外部 STL 引用** (v3.0) | 347 零件使 inline mesh 文件 >10MB，外部引用保持 XML 简洁 (~103KB) |
| **坐标处理** | **坐标本地化** (世界坐标→父 body 相对坐标) | 避免 MuJoCo 自动中心化导致惯性参数放大 10¹² 倍 |
| **零件质量** | **0.01-0.015kg** (body 0.01kg, arm 0.015kg) | 避免过高质量导致仿真不稳定，总质量 ~3.5kg |
| **执行器增益** | **kp=5, kv=2** (v2.0 为 kp=20, kv=1) | 抑制高频振动，平衡响应速度与稳定性 |
| **积分器** | **RK4** (仍然必要) | 347 个零件的复杂装配体对数值精度要求极高 |
